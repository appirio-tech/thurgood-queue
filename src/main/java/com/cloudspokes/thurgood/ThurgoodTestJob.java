...
// this test ensures that it passes if there are no participant or challenge. Just a job.
public class ThurgoodTestJob {

  private static String jobId = null;
  private static String codeUrl = "http://www.myfiles.com/code.zip";
  private static String language = "Java";
  private static String platform = "Heroku";
  ...
  public void testInit() {
    Thurgood t = new ThurgoodFactory().getTheJudge("Java");
    t.init(jobId);
    ...
    t.writeLog4jXmlFile();
    File log4j = new File("./src/main/webapp/WEB-INF/shells/java/log4j.xml");
    assertEquals(true, log4j.exists());
    
    t.writeBuildPropertiesFile();
    File buildFile = new File("./src/main/webapp/WEB-INF/shells/java/build.properties");
    assertEquals(true, buildFile.exists());  
    
    t.writeCloudspokesPropertiesFile();
    File cloudspokesFile = new File("./src/main/webapp/WEB-INF/shells/java/cloudspokes.properties");
    assertEquals(true, cloudspokesFile.exists());  
    // make sure the values are correct
    Properties cloudspokesProp = new Properties();
    try {
      //load a properties file
      cloudspokesProp.load(new FileInputStream("./src/main/webapp/WEB-INF/shells/java/cloudspokes.properties"));
      // make sure the values were set correctly
      assertEquals(jobId, cloudspokesProp.getProperty("s3_bucket"));
      ...
      // make sure challenge and participant were not added
      assertNull(cloudspokesProp.getProperty("challenge_id"));  
